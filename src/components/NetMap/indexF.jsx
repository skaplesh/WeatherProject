...
import UniqueDegree from "./components/UniqueDegreeSingle";
...
function SwitzerlandChoropleth({ data }) {
  ...
  useEffect(() => {
    ...
    // console.log(UniqueDegree(centrality_measures))
    ...
    if (mapInstance) {
      ...
      if (isD3Enabled) {
        ...
        users
          ...
          .attr('fill', (d) => {
            if (d.id) {
              console.log(MinMaxMap(userDegrees[d.id]))
  ...
  }, [data, leafletZoom, mapRef, isD3Enabled, hoveredUser]);
